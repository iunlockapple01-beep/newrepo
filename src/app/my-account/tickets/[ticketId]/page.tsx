...
export default function TicketDetailsPage() {
  ...
  const handleSendReply = async () => {
    ...
    const newReply = {
      ...
      createdAt: new Date().toISOString(), // Use simple string for arrayUnion if serverTimestamp is not allowed in nested objects
    };
    ...
    <div className="bg-gray-50 min-h-screen pb-12">
      ...
      <main className="max-w-4xl mx-auto pt-24 px-4 sm:px-6 lg:px-8">
        ...
        {(ticket.status === 'closed' || ticket.status === 'resolved') && (
            <p className="mt-4 text-center text-sm text-gray-500 italic">This ticket is marked as {ticket.status} and is no longer active.</p>
        )}
      </main>
    </div>
  );
